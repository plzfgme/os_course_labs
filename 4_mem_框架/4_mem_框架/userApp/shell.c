...
void addNewCmd(	unsigned char *cmd, 
    ...
    */
    if (ourCmds == NULL) {
        ourCmds = (struct cmd *)malloc(sizeof (struct cmd));
        strncpy(cmd, ourCmds->cmd, 21);
        ourCmds->func = func;
        ourCmds->help_func = help_func;
        strncpy(description, ourCmds->description, 101);
        ourCmds->nextCmd = NULL;
    } else {
        struct cmd * p = ourCmds;
        while (p->nextCmd != NULL)
        {
            p = p->nextCmd;
        }
        p->nextCmd = (struct cmd *)malloc(sizeof (struct cmd));
        strncpy(cmd, p->nextCmd->cmd, 21);
        p->nextCmd->func = func;
        p->nextCmd->help_func = help_func;
        strncpy(description, p->nextCmd->description, 101);
        p->nextCmd->nextCmd = NULL;
    }
}
